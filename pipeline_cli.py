...
from policy_app.pipelines.data_pipeline import data_pipeline
...
async def _async_main() -> None:
    ...
    art = await data_pipeline(args.seed_txt, args.pdf)

    if args.question:
        result = await rag_pipeline(art, args.question, top_k=args.top_k, alpha=args.alpha)
    ...
    try:
        while True:
            ...
            if not user_q:
                continue
            result = await rag_pipeline(art, user_q, top_k=args.top_k, alpha=args.alpha)
            print(_format_query_result(result))
